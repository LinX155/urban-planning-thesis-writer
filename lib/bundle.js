...
const SKILL_NAMES = ["uptw-init", "uptw-plan", "uptw-write"];
...
async function installCodex({ codexHome }) {
  ...
  for (const fileName of ["UPTW-init.md", "UPTW-plan.md", "UPTW-write.md"]) {
...
export async function packBundle(outputRoot) {
  ...
  await copyFiles(PATHS.codexPrompts, codexPromptsRoot);
  await copyFiles(PATHS.claudeCommands, claudeCommandsRoot);

  const manifest = {
    skills: [...SKILL_NAMES],
    codex_prompts: ["UPTW-init.md", "UPTW-plan.md", "UPTW-write.md"],
    claude_commands: ["UPTW-init.md", "UPTW-plan.md", "UPTW-write.md"],
